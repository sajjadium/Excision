...
// Excision <<<<
std::string *WebFrame::addExtensionScript(const std::string &source) {
	std::string *m_ext = NULL;

	if (toCoreFrame(this) != NULL && toCoreFrame(this)->domWindow()->document()->m_inclusion != NULL) {
		m_ext = new std::string(String(source.data()).stripWhiteSpace().utf8().data());
		toCoreFrame(this)->domWindow()->document()->m_inclusion->add(m_ext, NULL, "script", KURL(ParsedURLString, m_ext->data()));
	}

	return m_ext;
}

void WebFrame::startExecutingExtensionScript(const std::string *source) {
	if (source != NULL && toCoreFrame(this)->domWindow()->document()->m_inclusion != NULL)
		toCoreFrame(this)->domWindow()->document()->m_inclusion->startExecutingScript(source);
}

void WebFrame::finishExecutingExtensionScript(const std::string *source) {
	if (source != NULL && toCoreFrame(this)->domWindow()->document()->m_inclusion != NULL)
		toCoreFrame(this)->domWindow()->document()->m_inclusion->finishExecutingScript(source);
}
// Excision >>>>
