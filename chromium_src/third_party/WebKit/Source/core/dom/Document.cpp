...
// Excision <<<<
Classifier Inclusion::classifier;

HMM::HMM(const string &m_modelFilename) {
	this->filename = m_modelFilename;
}

HMM::~HMM() {
	delete[] m_stateProbabilities;

	for (int i = 0; i < m_numStates; i++) {
		delete[] m_transitionProbabilities[i];
		delete[] m_emissionProbabilities[i];
	}

	delete[] m_transitionProbabilities;
	delete[] m_emissionProbabilities;
}

double HMM::getProbability(const vector<int> &seq) {
	double** alpha = new double*[seq.size()];

	for (unsigned int j = 0; j < seq.size(); j++)
		alpha[j] = new double[m_numStates];

	for (int i = 0; i < m_numStates; i++)
		alpha[0][i] = m_stateProbabilities[i] * (seq[0] != -1 ? m_emissionProbabilities[i][seq[0]] : 0.0);
	...
	for (unsigned int j = 1; j < seq.size(); j++) {
		for (int i = 0; i < m_numStates; i++) {
			...
			for (int k = 0; k < m_numStates; k++)
				sum += alpha[j - 1][k] * m_transitionProbabilities[k][i];

			alpha[j][i] = sum * (seq[j] != -1 ? m_emissionProbabilities[i][seq[j]] : 0.0);
		}
	}

	double probability = 0.0;
	for (int i = 0; i < m_numStates; i++)
		probability += alpha[seq.size() - 1][i];
...
PublicSuffixListNode::PublicSuffixListNode(const string &m_name) {
	this->m_name = m_name;
}

PublicSuffixListNode::~PublicSuffixListNode() {
	for (map<string, PublicSuffixListNode *>::iterator it = m_children.begin(); it != m_children.end(); ++it)
		delete it->second;
}

PublicSuffixList::PublicSuffixList(const string &m_publicSuffixListFilename) {
	m_root = new PublicSuffixListNode("");

	fstream m_psFile(m_publicSuffixListFilename.data(), fstream::in);

	string line;
	while (!m_psFile.eof()) {
		getline(m_psFile, line);

		String ps = String(line.data()).stripWhiteSpace().lower();

		if (ps.isEmpty() || ps.startsWith("//"))
			continue;

		Vector<String> m_tokens;
		ps.split(".", m_tokens);

		PublicSuffixListNode *m_currentNode = m_root;

		for (int i = m_tokens.size() - 1; i >= 0; i--) {
			if (m_currentNode->m_children.count(m_tokens[i].utf8().data()) == 0)
				m_currentNode->m_children[m_tokens[i].utf8().data()] = new PublicSuffixListNode(m_tokens[i].utf8().data());

			m_currentNode = m_currentNode->m_children[m_tokens[i].utf8().data()];
		}
	}
...
string PublicSuffixList::getDomain(const string &domain) {
	Vector<String> m_tokens;
	String(domain.data()).split(".", m_tokens);
	...
	if (pos == m_tokens.size() - 1)
		return "";

	string ps = "";
	for (unsigned int i = pos; i < m_tokens.size(); i++)
		ps += string(".") + m_tokens[i].utf8().data();

	return ps.substr(1);
}

void *loadHMM(void *param) {
	HMM *m_hmm = (HMM *)param;

	fstream m_file(m_hmm->filename.data(), fstream::in);
	...
	// number of states
	getline(m_file, line);
	m_hmm->m_numStates = atoi(String(line.data()).lower().replace("nbstates", "").stripWhiteSpace().utf8().data());

	m_hmm->m_stateProbabilities = new double[m_hmm->m_numStates];

	m_hmm->m_transitionProbabilities = new double*[m_hmm->m_numStates];
	for (int i = 0; i < m_hmm->m_numStates; i++)
		m_hmm->m_transitionProbabilities[i] = new double[m_hmm->m_numStates];

	m_hmm->m_emissionProbabilities = new double*[m_hmm->m_numStates];
	...
	for (int i = 0; i < m_hmm->m_numStates; i++) {
		...
		// read Pi
		getline(m_file, line);
		m_hmm->m_stateProbabilities[i] = atof(String(line.data()).lower().replace("pi", "").stripWhiteSpace().utf8().data());

		// read A
		getline(m_file, line);
		Vector<String> m_aTokens;
		String(line.data()).lower().replace("a", "").stripWhiteSpace().split(" ", m_aTokens);
		for (int j = 0; j < m_hmm->m_numStates; j++) {
			m_hmm->m_transitionProbabilities[i][j] = atof(m_aTokens[j].utf8().data());
		}

		// read IntegerOPDF
		getline(m_file, line);
		Vector<String> m_bTokens;
		String(line.data()).lower().replace("integeropdf", "").replace("[", "").replace("]", "").stripWhiteSpace().split(" ", m_bTokens);
		m_hmm->m_numEmissions = m_bTokens.size();
		m_hmm->m_emissionProbabilities[i] = new double[m_hmm->m_numEmissions];
		for (int j = 0; j < m_hmm->m_numEmissions; j++)
			m_hmm->m_emissionProbabilities[i][j] = atof(m_bTokens[j].utf8().data());
...
void *loadEmissionMapping(void *params) {
	Classifier *m_classifier = (Classifier *)params;

	fstream m_emissionMappingFile(getenv("EMISSION_MAPPING_FILENAME"), fstream::in);

	string line;
	while (!m_emissionMappingFile.eof()) {
		getline(m_emissionMappingFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;
		...
		m_classifier->m_emissionMapping.add(m_tokens[0], atoi(m_tokens[1].utf8().data()));
	}

	m_emissionMappingFile.close();

	return NULL;
}

void *loadAlexaRanking(void *params) {
	Classifier *m_classifier = (Classifier *)params;

	fstream m_alexaRankingFile(getenv("ALEXA_RANKING_FILENAME"), fstream::in);

	string line;

	while (!m_alexaRankingFile.eof()) {
		getline(m_alexaRankingFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;
		...
		Vector<String> m_tokens;
		l.split("\t", m_tokens);
		m_classifier->m_alexaRanking.add(m_tokens[1], atoi(m_tokens[0].utf8().data()));
	}

	m_alexaRankingFile.close();

	return NULL;
}

Classifier::Classifier() : m_hmmBenign(getenv("HMM_MODEL_BENIGN_FILENAME")), m_hmmSuspicious(getenv("HMM_MODEL_SUSPICIOUS_FILENAME")), m_psl(getenv("PUBLIC_SUFFIX_FILENAME")), m_pslPrivate(getenv("PUBLIC_SUFFIX_PRIVATE_FILENAME")) {
	string line;

	// initialzie HMMs
	pthread_t hmm_benign_thread;
	pthread_create(&hmm_benign_thread, NULL, loadHMM, &m_hmmBenign);

	pthread_t hmm_suspicious_thread;
	pthread_create(&hmm_suspicious_thread, NULL, loadHMM, &m_hmmSuspicious);

	// initialzie emission mappings
	pthread_t emission_mapping_thread;
	pthread_create(&emission_mapping_thread, NULL, loadEmissionMapping, this);
	
	// initialzie alexa alexa_ranking
	pthread_t alexa_ranking_thread;
	pthread_create(&alexa_ranking_thread, NULL, loadAlexaRanking, this);

	// initialize ads
	fstream m_adsFile(getenv("ADS_FILENAME"), fstream::in);

	while (!m_adsFile.eof()) {
		getline(m_adsFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;

		m_ads.insert(l.utf8().data());
	}
	
	m_adsFile.close();

	// initialize url shortening services
	fstream m_ussFile(getenv("URL_SHORTENING_SERVICES_FILENAME"), fstream::in);

	while (!m_ussFile.eof()) {
		getline(m_ussFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;

		m_urlShortenerServices.insert(l.utf8().data());
	}

	m_ussFile.close();

	// initialize cdns
	fstream m_cdnFile(getenv("CDN_FILENAME"), fstream::in);

	while (!m_cdnFile.eof()) {
		getline(m_cdnFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;

		m_cdns.insert(getSLD(l.utf8().data()).data());
	}

	m_cdnFile.close();

	// initialize tld country international
	fstream m_tciFile(getenv("TLD_COUNTRY_INTERNATIONAL_FILENAME"), fstream::in);

	while (!m_tciFile.eof()) {
		getline(m_tciFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;
		...
		Vector<String> m_tokens;
		l.split("\t", m_tokens);
		m_tldCountryInternationals[m_tokens[0].utf8().data()] = m_tokens[1].utf8().data();
	}

	m_tciFile.close();

	// initialize feature classes
	fstream m_featureClassesFile(getenv("FEATURE_CLASSES_FILENAME"), fstream::in);

	while (!m_featureClassesFile.eof()) {
		getline(m_featureClassesFile, line);
		String l = String(line.data()).stripWhiteSpace().lower();
		if (l.isEmpty())
			continue;
		...
		if (m_tokens[2] == "1")
			m_continuousFeatures.insert(m_tokens[0].utf8().data());

		m_featureClasses[m_tokens[0].utf8().data()];

		Vector<String> m_tokens2;
		m_tokens[3].split(" ", m_tokens2);
		for (unsigned int i = 0; i < m_tokens2.size(); i++)
			m_featureClasses[m_tokens[0].utf8().data()].push_back(m_tokens2[i].utf8().data());
	}

	m_featureClassesFile.close();

	// initialize ip v4 private range
	m_privateIPv4Ranges.push_back(make_pair("127.0.0.1", "255.255.255.255"));
	m_privateIPv4Ranges.push_back(make_pair("10.0.0.0", "255.0.0.0"));
	m_privateIPv4Ranges.push_back(make_pair("172.16.0.0", "255.240.0.0"));
	m_privateIPv4Ranges.push_back(make_pair("192.168.0.0", "255.255.0.0"));

	m_genericTLDs.insert("com");
	m_genericTLDs.insert("org");
	m_genericTLDs.insert("net");
	m_genericTLDs.insert("edu");
	m_genericTLDs.insert("gov");
	m_genericTLDs.insert("mil");
	m_genericTLDs.insert("arpa");
	m_genericTLDs.insert("int");

	pthread_join(emission_mapping_thread, NULL);
	pthread_join(alexa_ranking_thread, NULL);
	pthread_join(hmm_benign_thread, NULL);
	pthread_join(hmm_suspicious_thread, NULL);
}

string Classifier::getHostnameWithoutPublicSuffix(const string &m_hostname) {
	string ps = getPublicSuffix(m_hostname);

	if (ps != "")
		return m_hostname.substr(0, m_hostname.length() - ps.length() - 1);

	return m_hostname;
}

string Classifier::getParentValue(double value) {
	if (value < 0)
		return "less";
	else if (value > 0)
		return "more";
...
int Classifier::minimum(int x, int y, int z) {
    if (x < y)
        return x < z ? x : z;
    else
        return y < z ? y : z;
...
int Classifier::editDistance(const string &A, const string &B) {
    int NA = A.size();
    int NB = B.size();
...
int Classifier::getUniqueCharacters(const string &str) {
	double chars[256];
	for (int i = 0; i < 256; i++)
		chars[i] = 0;

	for (unsigned int j = 0; j < str.length(); j++)
		chars[(int)str.data()[j]]++;
...
double Classifier::getCharacterCount(const string &str) {
	int chars[256];
	for (int i = 0; i < 256; i++)
		chars[i] = 0;
...
double Classifier::getNonAlphabetic(const string &str) {
	double m_nonAlpha = 0;
	for (unsigned int j = 0; j < str.length(); j++)
		if (isalpha(str.data()[j]) == 0)
			m_nonAlpha++;

	return m_nonAlpha / str.length();
}

Classifier::~Classifier() {
}

void calculateRanking(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	int rank = m_classifier->getRanking(m_hmmFeature->first["hostname"]);
	if (rank == -1)
		m_hmmFeature->first["alexa_ranking"] = "none";
	else
		m_hmmFeature->first["alexa_ranking"] = String::format("%.50f", rank / 1000000.0).utf8().data();
}

void calculateType(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["hostname_type"] = m_classifier->getHostnameType(m_hmmFeature->first["hostname"]);
	if (String(m_hmmFeature->first["hostname_type"].data()).startsWith("dns")) {
		if (m_classifier->getSLD(m_hmmFeature->first["hostname"]) != m_classifier->getSitename(m_hmmFeature->first["hostname"])) {
			if (m_hmmFeature->first["hostname"] == m_classifier->getSitename(m_hmmFeature->first["hostname"]))
				m_hmmFeature->first["hostname_type"] = "dns-non-sld";
			else
				m_hmmFeature->first["hostname_type"] = "dns-non-sld-subdomain";
		} else {
			if (m_hmmFeature->first["hostname"] == m_classifier->getSLD(m_hmmFeature->first["hostname"]))
				m_hmmFeature->first["hostname_type"] = "dns-sld";
			else
				m_hmmFeature->first["hostname_type"] = "dns-sld-subdomain";
		}
	}
}

void calculateTLD(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["tld_type"] = "none";
	if (m_classifier->getTLD(m_hmmFeature->first["hostname"]) != "") {
		if (m_classifier->m_genericTLDs.count(m_classifier->getTLD(m_hmmFeature->first["hostname"]).data()) > 0)
			m_hmmFeature->first["tld_type"] = "generic";
		else if (m_classifier->getTLD(m_hmmFeature->first["hostname"]).length() == 2)
			m_hmmFeature->first["tld_type"] = "country";
		else if (m_classifier->m_tldCountryInternationals.count(m_classifier->getTLD(m_hmmFeature->first["hostname"]).data()) > 0)
			m_hmmFeature->first["tld_type"] = "country-international";
		else
			m_hmmFeature->first["tld_type"] = "other";

		if (m_hmmFeature->first["tld_type"] == "generic" || m_hmmFeature->first["tld_type"] == "country")
			if (m_classifier->getPublicSuffix(m_hmmFeature->first["hostname"]) != m_classifier->getTLD(m_hmmFeature->first["hostname"]))
				m_hmmFeature->first["tld_type"] = m_hmmFeature->first["tld_type"] + "-subdomain";
	}
}

void calculateLen(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	if (m_hmmFeature->first["hostname_type"] == "ipv6")
		m_hmmFeature->first["hostname_len"] = String::format("%.50f", m_hmmFeature->first["hostname"].length() / 39.0).utf8().data();
	else if (String(m_hmmFeature->first["hostname_type"].data()).startsWith("ipv4"))
		m_hmmFeature->first["hostname_len"] = String::format("%.50f", m_hmmFeature->first["hostname"].length() / 15.0).utf8().data();
	else
		m_hmmFeature->first["hostname_len"] = String::format("%.50f", m_classifier->getHostnameWithoutPublicSuffix(m_hmmFeature->first["hostname"]).length() / 253.0).utf8().data();
}

void calculateNonAlphabetic(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["hostname_non_alphabetic"] = String::format("%.50f", m_classifier->getNonAlphabetic(m_classifier->getHostnameWithoutPublicSuffix(m_hmmFeature->first["hostname"]))).utf8().data();
}

void calculateCharFreq(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["hostname_char_freq"] = String::format("%.50f", m_classifier->getCharacterCount(m_classifier->getHostnameWithoutPublicSuffix(m_hmmFeature->first["hostname"]))).utf8().data();
}

void calculateUniqChars(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	if (m_hmmFeature->first["hostname_type"] == "ipv6")
		m_hmmFeature->first["hostname_uniq_chars"] = m_classifier->getUniqueCharacters(m_hmmFeature->first["hostname"]) / 17.0;
	else if (String(m_hmmFeature->first["hostname_type"].data()).startsWith("ipv4"))
		m_hmmFeature->first["hostname_uniq_chars"] = m_classifier->getUniqueCharacters(m_hmmFeature->first["hostname"]) / 11.0;
	else
		m_hmmFeature->first["hostname_uniq_chars"] = String::format("%.50f", m_classifier->getUniqueCharacters(m_classifier->getHostnameWithoutPublicSuffix(m_hmmFeature->first["hostname"])) / (26.0 + 10.0 + 1.0 + 1.0)).utf8().data();
}

void calculateEntropy(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	if (m_hmmFeature->first["hostname_type"] == "ipv6")
		m_hmmFeature->first["hostname_entropy"] = m_classifier->getEntropy(m_hmmFeature->first["hostname"]) / m_classifier->getEntropy(":abcdef0123456789");
	else if (String(m_hmmFeature->first["hostname_type"].data()).startsWith("ipv4"))
		m_hmmFeature->first["hostname_entropy"] = m_classifier->getEntropy(m_hmmFeature->first["hostname"]) / m_classifier->getEntropy(".0123456789");
	else
		m_hmmFeature->first["hostname_entropy"] = String::format("%.50f", m_classifier->getEntropy(m_classifier->getHostnameWithoutPublicSuffix(m_hmmFeature->first["hostname"])) / m_classifier->getEntropy("abcdefghijklmnopqrstuvwxyz0123456789-.")).utf8().data();
}

void calculateLevel(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["hostname_level"] = String::format("%.50f", (String(m_hmmFeature->first["hostname_type"].data()).startsWith("ip") ? 1.0 : std::count(m_hmmFeature->first["hostname"].data(), m_hmmFeature->first["hostname"].data() + m_hmmFeature->first["hostname"].length(), '.')) / 126.0).utf8().data();
}

void calculateAds(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["ads"] = (m_classifier->m_ads.count(m_hmmFeature->first["hostname"].data()) > 0 || m_classifier->m_ads.count(m_classifier->getSitename(m_hmmFeature->first["hostname"]).data()) > 0) ? "yes" : "no";
}

void calculateCDN(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["cdn"] = (m_classifier->m_cdns.count(m_classifier->getSLD(m_hmmFeature->first["hostname"]).data()) > 0) ? "yes" : "no";
}

void calculateUrlShorteningService(Classifier *m_classifier, HmmFeature *m_hmmFeature) {
	m_hmmFeature->first["url_shortening_service"] = (m_classifier->m_urlShortenerServices.count(m_hmmFeature->first["hostname"].data()) > 0 || m_classifier->m_urlShortenerServices.count(m_classifier->getSitename(m_hmmFeature->first["hostname"]).data()) > 0) ? "yes" : "no";
}

void Classifier::calculateHmmFeature(const vector<HmmFeature *> &m_hmmFeaturePath, const string &m_inclusion, HmmFeature &m_hmmFeature) {
	string m_hostname = getHostname(m_inclusion);

	if (m_hostname.empty() || (m_hmmFeaturePath.size() > 0 && m_hostname == m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"])) {
		m_hmmFeature.second = -1;
		return;
	}

	m_hmmFeature.first["hostname"] = m_hostname;

	// Alexa Ranking
	calculateRanking(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateRanking(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["alexa_ranking_parent"] = "none";
	if (m_hmmFeaturePath.size() > 0) {
		if (m_hmmFeature.first["alexa_ranking"] != "none" && m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["alexa_ranking"] != "none")
			m_hmmFeature.first["alexa_ranking_parent"] = getParentValue(atof(m_hmmFeature.first["alexa_ranking"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["alexa_ranking"].data()));
		else if (m_hmmFeature.first["alexa_ranking"] != "none" && m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["alexa_ranking"] == "none")
			m_hmmFeature.first["alexa_ranking_parent"] = "less";
		else if (m_hmmFeature.first["alexa_ranking"] == "none" && m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["alexa_ranking"] != "none")
			m_hmmFeature.first["alexa_ranking_parent"] = "more";
	}

	// Hostname Type
	calculateType(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateType(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_type_parent"] = "none";
	if (m_hmmFeaturePath.size() > 0) {
		if (getSitename(m_hostname) != "" && getSitename(m_hostname) == getSitename(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"]))
			m_hmmFeature.first["hostname_type_parent"] = "same-website";
		else if (getSLD(m_hostname) != "" && getSLD(m_hostname) == getSLD(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"]))
			m_hmmFeature.first["hostname_type_parent"] = "same-sld";
		else if (getPublicSuffix(m_hostname) != "" && getPublicSuffix(m_hostname) == getPublicSuffix(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"]))
			m_hmmFeature.first["hostname_type_parent"] = "same-effective-tld";
		else if (getTLD(m_hostname) != "" && getTLD(m_hostname) == getTLD(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"]))
			m_hmmFeature.first["hostname_type_parent"] = "same-tld";
		else if (getCompany(m_hostname) != "" && getCompany(m_hostname) == getCompany(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"]))
			m_hmmFeature.first["hostname_type_parent"] = "same-company";
		else
			m_hmmFeature.first["hostname_type_parent"] = String::format("different-%s-to-%s", String(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_type"].data()).startsWith("dns") ? "dns" : "ip", String(m_hmmFeature.first["hostname_type"].data()).startsWith("dns") ? "dns" : "ip").utf8().data();
	}

	// TLD
	calculateTLD(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateTLD(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["tld_type_parent"] = "none";
	if (m_hmmFeaturePath.size() > 0) {
		if (m_hmmFeature.first["tld_type"] != "none" && m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["tld_type"] == "none")
			m_hmmFeature.first["tld_type_parent"] = "got-tld";
		else if (m_hmmFeature.first["tld_type"] == "none" && m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["tld_type"] != "none")
			m_hmmFeature.first["tld_type_parent"] = "lost-tld";
		else if (m_hmmFeature.first["tld_type"] != "none" && m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["tld_type"] != "none") {
			Vector<String> m_featureTokens;
			String(m_hmmFeature.first["tld_type"].data()).split("-", m_featureTokens);

			Vector<String> m_parentFeatureTokens;
			String(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["tld_type"].data()).split("-", m_parentFeatureTokens);

			if (m_featureTokens[0] == m_parentFeatureTokens[0]) {
				if (getTLDNonInternational(m_hostname) == getTLDNonInternational(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname"]))
					m_hmmFeature.first["tld_type_parent"] = string("same-") + m_featureTokens[0].utf8().data();
				else
					m_hmmFeature.first["tld_type_parent"] = string("different-") + m_featureTokens[0].utf8().data();
			} else
				m_hmmFeature.first["tld_type_parent"] = m_parentFeatureTokens[0].utf8().data() + string("-to-") + m_featureTokens[0].utf8().data();
		}
	}

	// Len
	calculateLen(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateLen(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_len_parent"] = m_hmmFeaturePath.size() > 0 ? getParentValue(atof(m_hmmFeature.first["hostname_len"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_len"].data())) : "none";

	// Non alphabetic
	calculateNonAlphabetic(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateNonAlphabetic(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_non_alphabetic_parent"] = m_hmmFeaturePath.size() > 0 ? getParentValue(atof(m_hmmFeature.first["hostname_non_alphabetic"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_non_alphabetic"].data())) : "none";

	// Character Freq
	calculateCharFreq(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateCharFreq(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_char_freq_parent"] = m_hmmFeaturePath.size() > 0 ? getParentValue(atof(m_hmmFeature.first["hostname_char_freq"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_char_freq"].data())) : "none";

	// Unique Chars
	calculateUniqChars(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateUniqChars(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_uniq_chars_parent"] = m_hmmFeaturePath.size() > 0 ? getParentValue(atof(m_hmmFeature.first["hostname_uniq_chars"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_uniq_chars"].data())) : "none";

	// Entropy
	calculateEntropy(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateEntropy(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_entropy_parent"] = m_hmmFeaturePath.size() > 0 ? getParentValue(atof(m_hmmFeature.first["hostname_entropy"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_entropy"].data())) : "none";

	// Level
	calculateLevel(this, &m_hmmFeature);
//	if (m_hmmFeaturePath.size() > 0)
//		calculateLevel(this, m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]);

	m_hmmFeature.first["hostname_level_parent"] = m_hmmFeaturePath.size() > 0 ? getParentValue(atof(m_hmmFeature.first["hostname_level"].data()) - atof(m_hmmFeaturePath[m_hmmFeaturePath.size() - 1]->first["hostname_level"].data())) : "none";

	// Ads
	calculateAds(this, &m_hmmFeature);
//	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++)
//		calculateAds(this, m_hmmFeaturePath[j]);

	string m_hasAncestors = "no";
	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++)
		if (m_hmmFeaturePath[j]->first["ads"] == "yes") {
			m_hasAncestors = "yes";
			break;
		}

	m_hmmFeature.first["ads_ancestors"] = m_hmmFeaturePath.size() == 0 ? "none" : m_hasAncestors;

	// CDN
	calculateCDN(this, &m_hmmFeature);
//	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++)
//		calculateCDN(this, m_hmmFeaturePath[j]);

	m_hasAncestors = "no";
	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++)
		if (m_hmmFeaturePath[j]->first["cdn"] == "yes") {
			m_hasAncestors = "yes";
			break;
		}

	m_hmmFeature.first["cdn_ancestors"] = m_hmmFeaturePath.size() == 0 ? "none" : m_hasAncestors;

	// URL Shortening Service
	calculateUrlShorteningService(this, &m_hmmFeature);
//	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++)
//		calculateUrlShorteningService(this, m_hmmFeaturePath[j]);

	m_hasAncestors = "no";
	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++)
		if (m_hmmFeaturePath[j]->first["url_shortening_service"] == "yes") {
			m_hasAncestors = "yes";
			break;
		}

	m_hmmFeature.first["url_shortening_service_ancestors"] = m_hmmFeaturePath.size() == 0 ? "none" : m_hasAncestors;

	vector<int> m_featureVector;

	for (map<string, vector<string> >::iterator it = m_featureClasses.begin(); it != m_featureClasses.end(); ++it) {
		if (m_continuousFeatures.count(it->first) > 0) {
			if (m_featureClasses[it->first][0] == "none" && m_hmmFeature.first[it->first] == "none")
				m_featureVector.push_back(0);
			else {
				for (unsigned int k = (int)(m_featureClasses[it->first][0] == "none"); k < m_featureClasses[it->first].size(); k++) {
					if (atof(m_hmmFeature.first[it->first].data()) <= atof(m_featureClasses[it->first][k].data())) {
						m_featureVector.push_back(k);
						break;
					}
				}
			}
		} else {
			m_featureVector.push_back(find(m_featureClasses[it->first].begin(), m_featureClasses[it->first].end(), m_hmmFeature.first[it->first]) - m_featureClasses[it->first].begin());
		}
	}

	stringstream m_valueStream;

	for (unsigned int j = 0; j < m_featureVector.size(); j++)
		m_valueStream << "," << m_featureVector[j];

	string m_value = m_valueStream.str().substr(1);

	if (m_emissionMapping.contains(String(m_value.data())))
		m_hmmFeature.second = -1;
	else
		m_hmmFeature.second = m_emissionMapping.get(String(m_value.data()));

//	m_hmmFeature.first.clear();
//	m_hmmFeature.first["hostname"] = m_hostname;

//	for (unsigned int j = 0; j < m_hmmFeaturePath.size(); j++) {
//		string h = m_hmmFeaturePath[j]->first["hostname"];
//		m_hmmFeaturePath[j]->first.clear();
//		m_hmmFeaturePath[j]->first["hostname"] = h;
//	}
}

bool Classifier::isSuspicious(const vector<HmmFeature *> &m_hmmFeaturePath) {
	vector<string> m_compactSites;
	for (unsigned int i = 0; i < m_hmmFeaturePath.size(); i++)
		if (m_compactSites.size() == 0 || m_compactSites.back() != getSitename(m_hmmFeaturePath[i]->first["hostname"]))
			m_compactSites.push_back(getSitename(m_hmmFeaturePath[i]->first["hostname"]));

	if (m_compactSites.size() < 3)
		return false;

	if (m_compactSites[0] == m_compactSites.back() || m_compactSites[0] == m_compactSites[m_compactSites.size() - 2])
		return false;

//	for (unsigned int i = 0; i < m_hmmFeaturePath.size(); i++)
//		cerr << m_hmmFeaturePath[i]->first["hostname"] << ";";
//	cerr << "\t";

//	for (unsigned int i = 0; i < m_hmmFeaturePath.size(); i++)
//		cerr << m_hmmFeaturePath[i]->second << ";";
...
//	cerr << endl;
	...
	m_hmmSeq.push_back(0);

	for (unsigned int i = 0; i < m_hmmFeaturePath.size(); i++)
		m_hmmSeq.push_back(m_hmmFeaturePath[i]->second);

	m_hmmSeq.push_back(0);

	double m_suspiciousProbability = m_hmmSuspicious.getProbability(m_hmmSeq);
	double m_benignProbability = m_hmmBenign.getProbability(m_hmmSeq);

//	cerr << "P(Benign) = " << m_benignProbability << ", P(Suspicious) = " << m_suspiciousProbability << ", Result = " << (m_suspiciousProbability >= m_benignProbability) << endl;

	return m_suspiciousProbability >= m_benignProbability;
}

string Classifier::getTLDNonInternational(const string &m_domain) {
	string tld = getTLD(m_domain);

	if (m_tldCountryInternationals.count(tld.data()) > 0)
		tld = m_tldCountryInternationals[tld.data()];

	return tld;
}

string Classifier::getHostnameType(const string &m_hostname) {
	unsigned char ptonbuf[sizeof(struct in6_addr)];

	if (inet_pton(AF_INET, m_hostname.data(), ptonbuf) == 1) {
		for (unsigned int j = 0; j < m_privateIPv4Ranges.size(); j++)
			if ((inet_addr(m_hostname.data()) & inet_addr(m_privateIPv4Ranges[j].second.data())) == inet_addr(m_privateIPv4Ranges[j].first.data()))
				return "ipv4-private";

		return "ipv4-public";
	}

	if (inet_pton(AF_INET6, m_hostname.data(), ptonbuf) == 1)
		return "ipv6";

	return "dns";
}

string Classifier::getHostname(const string &m_url) {
	return KURL(ParsedURLString, String(m_url.data()).stripWhiteSpace().lower()).host().stripWhiteSpace().utf8().data();
}

string Classifier::getSitename(const string &domain) {
	if (getHostnameType(domain) == "dns") {
		String d = String(m_pslPrivate.getDomain(domain).data());

		if (d.contains(".") && !d.startsWith(".") && !d.endsWith("."))
			return d.utf8().data();
	}
...
string Classifier::getSLD(const string &domain) {
	if (getHostnameType(domain) == "dns") {
		String d = String(m_psl.getDomain(domain).data());
...
string Classifier::getPublicSuffix(const string &domain) {
	string sld = getSLD(domain);

	if (sld != "") {
		Vector<String> m_tokens;
		String(sld.data()).split(".", m_tokens);
		string ps = "";
		for (unsigned int i = 1; i < m_tokens.size(); i++)
			ps += string(".") + m_tokens[i].utf8().data();
...
string Classifier::getTLD(const string &domain) {
	string ps = getPublicSuffix(domain);

	if (ps != "") {
		Vector<String> m_tokens;
		String(ps.data()).split(".", m_tokens);

		return m_tokens[m_tokens.size() - 1].utf8().data();
	}

	return "";
}

string Classifier::getCompany(const string &domain) {
	string sld = getSLD(domain);
	...
	if (sld != "") {
		...
		return m_tokens[0].utf8().data();
	}

	return "";
}

int Classifier::getRanking(const string &domain) {
	if (m_alexaRanking.contains(String(domain.data())))
		return m_alexaRanking.get(String(domain.data()));

	if (m_alexaRanking.contains(String(getSitename(domain).data())))
		return m_alexaRanking.get(String(getSitename(domain).data()));

	return -1;
}

double Classifier::getEntropy(const string &str) {
	...
	for (int i = 0; i < 256; i++) {
		...
		double p = (double)freq[i] / strlen(chars);
		ent += -p * log2(p);
	}
...
InclusionNode::InclusionNode(int m_id, const void *m_this, const void *m_parent, const string &m_type, const KURL &m_url) {
	this->m_id = m_id;
	this->m_this = m_this;
...
Inclusion::Inclusion(Document *m_document, const KURL &m_documentUrl) {
	if (m_documentUrl.isEmpty() || (m_document->ownerElement() != NULL && static_cast<HTMLFrameElementBase *>(m_document->ownerElement()) == NULL) || (m_document->url() == blankURL() && m_document->ownerElement() == NULL)) {
		this->m_document = NULL;
		m_parentDocument = NULL;
		m_parentFrameNode = NULL;
	} else {
		this->m_document = m_document;
		this->m_documentUrl = KURL(ParsedURLString, m_documentUrl.string().stripWhiteSpace().lower());

		m_parentDocument = m_document->parentDocument();
		m_parentFrameNode = m_document->ownerElement();

		if (m_parentDocument != NULL) {
			m_parentDocument->m_inclusion->getHmmFeaturePath(m_parentFrameNode, m_documentHmmFeaturePath);
		}
/*
		if (m_parentDocument == NULL && (m_documentUrl.string().startsWith("http://") || m_documentUrl.string().startsWith("https://"))) {
			FILE *m_pagesFile = fopen("webpages.txt", "a");
			fprintf(m_pagesFile, "%s\n", m_documentUrl.string().utf8().data());
			fclose(m_pagesFile);
		}
*/
		classifier.calculateHmmFeature(m_documentHmmFeaturePath, m_documentUrl.string().utf8().data(), m_documentHmmFeature);
		if (m_documentHmmFeature.second != -1) {
			m_documentHmmFeaturePath.push_back(&m_documentHmmFeature);
		}
	}

	m_extensionEventId = 0;

	m_timestamp = time(NULL);
	m_isParsingFinished = false;
...
Inclusion::~Inclusion() {
	...
	for (map<const void *, InclusionNode *>::iterator it = m_inclusionNodes.begin(); it != m_inclusionNodes.end(); ++it)
		delete it->second;
...
string Inclusion::nodeToString(const InclusionNode *m_in) {
	stringstream ss;

	ss << m_in->m_this << "\t";
	if (m_in->m_parent != NULL)
		ss << m_in->m_parent;
	else
		ss << "(nil)";

	String m_urlString = m_in->m_url.string().stripWhiteSpace();

	if (m_urlString.startsWith("data:") || m_urlString.isEmpty())
		m_urlString = (string("inline-") + m_in->m_type).data();

	ss << "\t" << m_in->m_type << "\t" << m_urlString.utf8().data();

	return ss.str();
}

string Inclusion::sha256(const string &data) {
	DigestValue digest;
	computeDigest(HashAlgorithmSha256, data.data(), data.length(), digest);
...
void Inclusion::addTimer(const void *m_timer) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_executingScripts.size() == 0) {
		pthread_mutex_unlock(&m_lock);
		return;
	}

	m_timers[m_timer] = m_executingScripts.front()->m_this;

	pthread_mutex_unlock(&m_lock);
}

const void *Inclusion::findExtensionAncestor(const void *m_element) {
	if (m_inclusionNodes.count(m_element) > 0) {
		if (m_inclusionNodes[m_element]->m_url.string().lower().startsWith("chrome-extension:"))
			return m_element;
		else {
			if (m_inclusionNodes[m_element]->m_parent != NULL)
				return findExtensionAncestor(m_inclusionNodes[m_element]->m_parent);
			else if (m_parentDocument && m_parentDocument->m_inclusion && m_parentFrameNode)
				return m_parentDocument->m_inclusion->findExtensionAncestor(m_parentFrameNode);
		}
	}
...
void Inclusion::updateEvent(const void *m_element) {
	if (m_eventListenerScriptMapping.count(m_element) == 0)
		return;

	vector<pair<string, void *> > m_elems;
	EventTarget *m_evtTarget = (EventTarget *)m_element;

	if ((m_evtTarget->toDOMWindow() != NULL) || (m_evtTarget->toNode() != NULL && m_evtTarget->toNode()->isDocumentNode()) || (m_evtTarget->toNode() != NULL && m_evtTarget->toNode()->isElementNode() && ((Element *)m_element)->tagName().lower() == "html")) {
		m_elems.push_back(make_pair("window", m_document->domWindow()));
		m_elems.push_back(make_pair("document", m_document));
		m_elems.push_back(make_pair("", m_document->documentElement()));
	} else if (m_evtTarget->toNode() != NULL && m_evtTarget->toNode()->isElementNode())
		m_elems.push_back(make_pair("", (void *)m_element));

	if (m_elems.size() == 0 || (Element *)m_elems.back().second == NULL)
		return;

	int extEvtId = -1;
	if (((Element *)m_elems.back().second)->hasAttribute(QualifiedName(nullAtom, "excision-ext-events-id", "")))
		extEvtId = atoi(((Element *)m_elems.back().second)->getAttribute(QualifiedName(nullAtom, "excision-ext-events-id", "")).utf8().data());

	unordered_set<string> ext_events;
	for (unsigned int i = 0; i < m_elems.size(); i++) {
		for (map<string, vector<pair<const void *, const void *> > >::iterator it = m_eventListenerScriptMapping[m_elems[i].second].begin(); it != m_eventListenerScriptMapping[m_elems[i].second].end(); ++it) {
			string evt_name = it->first;
			if (!m_elems[i].first.empty())
				evt_name = m_elems[i].first + ":" + evt_name;

			for (unsigned int j = 0; j < it->second.size(); j++) {
				if (findExtensionAncestor(it->second[i].second) != NULL) {
					ext_events.insert(evt_name);
					break;
				}
			}
		}
	...
	if (ext_events.size() > 0) {
		if (extEvtId == -1)
			extEvtId = m_extensionEventId++;

		stringstream ss;

		ss << extEvtId;

		((Element *)m_elems.back().second)->setAttribute(QualifiedName(nullAtom, "excision-ext-events-id", ""), ss.str().data());

		ss.str("");

		unordered_set<string>::iterator it = ext_events.begin();

		ss << *it;

		++it;

		while (it != ext_events.end()) {
			ss << " " << *it;
			++it;
		}

		((Element *)m_elems.back().second)->setAttribute(QualifiedName(nullAtom, "excision-ext-events", ""), ss.str().data());
	}
}

void Inclusion::addEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_eventListenerScriptMapping.count(m_element) == 0)
		m_eventListenerScriptMapping[m_element];

	if (m_eventListenerScriptMapping[m_element].count(m_eventType) == 0)
		m_eventListenerScriptMapping[m_element][m_eventType];

	m_eventListenerScriptMapping[m_element][m_eventType].push_back(make_pair(m_listener, m_executingScripts.front()->m_this));

	updateEvent(m_element);

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::removeEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_eventListenerScriptMapping.count(m_element) > 0 && m_eventListenerScriptMapping[m_element].count(m_eventType) > 0) {
		for (unsigned int i = 0; i < m_eventListenerScriptMapping[m_element][m_eventType].size(); i++)
			if (m_eventListenerScriptMapping[m_element][m_eventType][i].first == m_listener) {
				m_eventListenerScriptMapping[m_element][m_eventType].erase(m_eventListenerScriptMapping[m_element][m_eventType].begin() + i);
				break;
			}

		if (m_eventListenerScriptMapping[m_element][m_eventType].size() == 0) {
			m_eventListenerScriptMapping[m_element].erase(m_eventType);

			if (m_eventListenerScriptMapping[m_element].size() == 0)
				m_eventListenerScriptMapping.erase(m_element);
		}
	}
...
bool Inclusion::sort(InclusionNode *m_in1, InclusionNode* m_in2) {
	return m_in1->m_id < m_in2->m_id;
}

void Inclusion::print() {
	pthread_mutex_lock(&m_lock);
	...
	if (!m_isParsingFinished) {
		pthread_mutex_unlock(&m_lock);
		return;
	}

	if (m_documentUrl == blankURL() && m_parentDocument->m_inclusion->m_inclusionNodes[m_parentFrameNode]->m_url != m_documentUrl) {
		pthread_mutex_unlock(&m_lock);
		return;
	}

	// sort inclusion nodes
	vector<InclusionNode *> m_sortedInclusionNodes;
	for (map<const void *, InclusionNode *>::iterator it = m_inclusionNodes.begin(); it != m_inclusionNodes.end(); ++it)
		m_sortedInclusionNodes.push_back(it->second);

	std::sort(m_sortedInclusionNodes.begin(), m_sortedInclusionNodes.end(), Inclusion::sort);

	// write inclusions to file
	stringstream m_ss;
	m_ss << m_timestamp << " " << m_document << " " << m_documentUrl.string().utf8().data();

	stringstream m_filename;
	m_filename << getenv("LOGS_DIR") << "/" << getenv("EXTENSION_NAME") << "_" << getenv("INITIAL_DOMAIN") << "_" << sha256(m_ss.str()) << ".log";

	fstream m_logfile(m_filename.str(), fstream::out | fstream::trunc);
	m_logfile << getenv("INITIAL_DOMAIN") << "\t" << getenv("EXTENSION_NAME") << "\t" << m_timestamp << "\t" << m_document << "\t" << m_documentUrl.string().utf8().data();

	m_logfile << "\t" << (m_document->referrer().isEmpty() ? "(nil)" : m_document->referrer().utf8().data());

	if (m_parentDocument == NULL)
		m_logfile << "\t" << "(nil)";
	else
		m_logfile << "\t" << m_parentDocument;

	m_logfile << "\t";

	if (m_parentFrameNode != NULL)
		m_logfile << m_parentFrameNode;
	else
		m_logfile << "(nil)";

	// for redirector
	m_logfile << "\t(nil)\t(nil)\t(nil)\n";

	m_logfile << "\n\n";

	for (unsigned int i = 0; i < m_sortedInclusionNodes.size(); i++) {
		m_logfile << nodeToString(m_sortedInclusionNodes[i]) << "\n";
	}
...
void Inclusion::addAttribute(const void *m_this, const string &m_name, const string &m_value) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_inclusionNodes.count(m_this) > 0)
		m_inclusionNodes[m_this]->m_attributes[m_name] = m_value;

	pthread_mutex_unlock(&m_lock);
}

bool Inclusion::add(const void *m_this, const void *m_parent, const string &m_type, const KURL &m_url) {
	pthread_mutex_lock(&m_lock);
	...
	InclusionNode *m_in = NULL;

	if (m_inclusionNodes.count(m_this) == 0) {
		if (m_executingScripts.size() == 0) {
			for (unsigned int i = 0; i < m_preloadQueue.size(); i++)
				if (m_preloadQueue[i]->m_type == "" && m_preloadQueue[i]->m_url == m_url) {
					m_in = m_preloadQueue[i];

					m_in->m_this = m_this;
					m_in->m_type = m_type;

					if (m_in->m_type != "script")
			...
			if (m_in == NULL) {
				m_in = new InclusionNode(m_inclusionNodes.size(), m_this, m_parent, m_type, m_url);
				if (m_in->m_type == "script")
					m_preloadQueue.push_back(m_in);
			}
		} else {
			m_in = new InclusionNode(m_inclusionNodes.size(), m_this, m_parent == NULL ? m_executingScripts.front()->m_this : m_parent, m_type, m_url);
			if (m_in->m_type == "script")
				m_executionQueue.push_back(m_in);
		}

		m_inclusionNodes[m_in->m_this] = m_in;

		if (m_inclusionNodes.count(m_in->m_parent) == 0)
			m_in->m_parent = NULL;
	} else {
		m_in = m_inclusionNodes[m_this];

		if (m_in->m_url != m_url) {
			m_in->m_url = m_url;
			if (m_executingScripts.size() > 0) {
				m_in->m_parent = m_executingScripts.front()->m_this;
			}
		}
	}

	if (m_in->m_url.string().startsWith("chrome-extension") && m_in->m_parent != NULL && m_inclusionNodes.count(m_in->m_parent) > 0 && !m_inclusionNodes[m_in->m_parent]->m_url.string().startsWith("chrome-extension"))
		m_in->m_parent = NULL;

//	bool m_isInWhiteList = m_document->contentSecurityPolicy()->allowInclusion(m_in->m_type.data(), m_in->m_url);
//	m_isInWhiteList = true;

	bool m_isSuspicious = false;

	if (m_in->m_parent != NULL)
		m_in->m_hmmFeaturePath.insert(m_in->m_hmmFeaturePath.end(), m_inclusionNodes[m_in->m_parent]->m_hmmFeaturePath.begin(), m_inclusionNodes[m_in->m_parent]->m_hmmFeaturePath.end());
	else
		m_in->m_hmmFeaturePath.insert(m_in->m_hmmFeaturePath.end(), m_documentHmmFeaturePath.begin(), m_documentHmmFeaturePath.end());

//		getHmmFeaturePath(m_in->m_this, m_hmmFeaturePath);

	classifier.calculateHmmFeature(m_in->m_hmmFeaturePath, m_in->m_url.string().utf8().data(), m_in->m_hmmFeature);

	if (m_in->m_hmmFeature.second != -1) {
		m_in->m_hmmFeaturePath.push_back(&m_in->m_hmmFeature);

		m_isSuspicious = classifier.isSuspicious(m_in->m_hmmFeaturePath);
		m_isSuspicious = false;

		if (m_isSuspicious) {
			if (m_in->m_url.host() != "") {
				FILE *m_maliciousHostnamesFile = fopen("malicious-hostnames.txt", "a");
				fprintf(m_maliciousHostnamesFile, "%s\n", m_in->m_url.host().utf8().data());
				fclose(m_maliciousHostnamesFile);
			}

			m_inclusionNodes.erase(m_in->m_this);

			vector<InclusionNode *>::iterator it = find(m_preloadQueue.begin(), m_preloadQueue.end(), m_in);
			if (it != m_preloadQueue.end())
				m_preloadQueue.erase(it);

			it = find(m_executionQueue.begin(), m_executionQueue.end(), m_in);
...
void Inclusion::getHmmFeaturePath(const void *m_this, vector<HmmFeature *> &m_hmmFeaturePath) {
	pthread_mutex_lock(&m_lock);
	...
	InclusionNode *m_in = m_inclusionNodes[m_this];

	while (m_in->m_parent != NULL) {
		InclusionNode *m_parentIn = m_inclusionNodes[m_in->m_parent];

		if (m_parentIn->m_hmmFeature.second != -1)
			m_hmmFeaturePath.insert(m_hmmFeaturePath.begin(), &m_parentIn->m_hmmFeature);

		m_in = m_parentIn;
	}

	m_hmmFeaturePath.insert(m_hmmFeaturePath.begin(), m_documentHmmFeaturePath.begin(), m_documentHmmFeaturePath.end());

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::preload(const string &m_url) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_executionQueue.size() > 0 && m_executionQueue.back()->m_type == "script" && m_executionQueue.back()->m_url == m_url.data()) {
		pthread_mutex_unlock(&m_lock);
		return;
	}

	InclusionNode *rn = new InclusionNode(m_inclusionNodes.size(), NULL, m_executingScripts.front()->m_this, "", KURL(ParsedURLString, m_url.data()));
	m_executionQueue.push_back(rn);

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::startExecutingScript(const void *m_this) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_executingScripts.size() > 0) {
		m_preloadQueue.insert(find(m_preloadQueue.begin(), m_preloadQueue.end(), m_executingScripts.front()), m_executionQueue.begin(), m_executionQueue.end());
		m_executionQueue.clear();
	}

	m_executingScripts.insert(m_executingScripts.begin(), m_in);

	if (std::count(m_preloadQueue.begin(), m_preloadQueue.end(), m_in) < std::count(m_executingScripts.begin(), m_executingScripts.end(), m_in))
		m_preloadQueue.insert(m_preloadQueue.begin(), m_in);

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::finishExecutingScript(const void *m_this) {
	pthread_mutex_lock(&m_lock);
	...
	m_preloadQueue.insert(m_preloadQueue.erase(find(m_preloadQueue.begin(), m_preloadQueue.end(), m_in)), m_executionQueue.begin(), m_executionQueue.end());
	m_executionQueue.clear();

	m_executingScripts.erase(m_executingScripts.begin());

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::startExecutingTimer(const void *m_timer) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_timers.count(m_timer) == 0) {
		pthread_mutex_unlock(&m_lock);
		return;
	}

	startExecutingScript(m_timers[m_timer]);

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::finishExecutingTimer(const void *m_timer) {
	pthread_mutex_lock(&m_lock);
	...
	finishExecutingScript(m_timers[m_timer]);

	pthread_mutex_unlock(&m_lock);
}

void Inclusion::startExecutingEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_eventListenerScriptMapping.count(m_element) > 0 && m_eventListenerScriptMapping[m_element].count(m_eventType) > 0)
		for (unsigned int i = 0; i < m_eventListenerScriptMapping[m_element][m_eventType].size(); i++)
			if (m_eventListenerScriptMapping[m_element][m_eventType][i].first == m_listener) {
				startExecutingScript(m_eventListenerScriptMapping[m_element][m_eventType][i].second);
				break;
			}
...
void Inclusion::finishExecutingEvent(const void *m_element, const string &m_eventType, const void *m_listener) {
	pthread_mutex_lock(&m_lock);
	...
	if (m_eventListenerScriptMapping.count(m_element) > 0 && m_eventListenerScriptMapping[m_element].count(m_eventType) > 0)
		for (unsigned int i = 0; i < m_eventListenerScriptMapping[m_element][m_eventType].size(); i++)
			if (m_eventListenerScriptMapping[m_element][m_eventType][i].first == m_listener) {
				finishExecutingScript(m_eventListenerScriptMapping[m_element][m_eventType][i].second);
				break;
			}
    ...
    , m_parserSyncPolicy(AllowAsynchronousParsing)
{
	...
	// Excision <<<<
	m_inclusion = new Inclusion(this, url());
	// Excision >>>>
}
...
void Document::finishedParsing()
{
	...
	// Excision <<<<
	if (m_inclusion != NULL) {
		m_inclusion->m_isParsingFinished = true;
		m_inclusion->print();
	}
	// Excision >>>>
}
