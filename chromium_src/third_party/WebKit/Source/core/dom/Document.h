...
// Excision <<<<
typedef pair<map<string, string>, int> HmmFeature;
...
public:
	...
	vector<HmmFeature *> m_hmmFeaturePath;
	HmmFeature m_hmmFeature;

	InclusionNode(int m_id, const void *, const void *, const string &, const KURL &);
};
...
public:
	...
	double* m_stateProbabilities;
	double** m_transitionProbabilities;
	double** m_emissionProbabilities;

	HMM(const string &);
	~HMM();

	double getProbability(const vector<int> &);
};

class PublicSuffixListNode {
public:
	string m_name;
	map<string, PublicSuffixListNode *> m_children;

	PublicSuffixListNode(const string &m_name);
	~PublicSuffixListNode();
};

class PublicSuffixList {
public:
	PublicSuffixListNode *m_root;

	PublicSuffixList(const string &);
	~PublicSuffixList();
...
public:
	...
	PublicSuffixList m_psl;
	PublicSuffixList m_pslPrivate;

	set<string> m_continuousFeatures;
	map<string, vector<string> > m_featureClasses;

	HashMap<String, int> m_emissionMapping;

	HashMap<String, int> m_alexaRanking;

	set<string> m_ads;
	set<string> m_urlShortenerServices;
	set<string> m_cdns;
	map<string, string> m_tldCountryInternationals;
	set<string> m_genericTLDs;

	vector<pair<string, string> > m_privateIPv4Ranges;

	Classifier();
	~Classifier();

	void calculateHmmFeature(const vector<HmmFeature *> &, const string &, HmmFeature &);

	bool isSuspicious(const vector<HmmFeature *> &);

	string getHostnameType(const string &);
	string getHostname(const string &);
	string getSitename(const string &);
	string getSLD(const string &);
	string getPublicSuffix(const string &);
	string getTLD(const string &);
	string getCompany(const string &);
	int getRanking(const string &);
	double getEntropy(const string &);
	string getTLDNonInternational(const string &);
	int minimum(int, int, int);
	int editDistance(const string &, const string &);
	int getUniqueCharacters(const string &);
	double getCharacterCount(const string &);
	double getNonAlphabetic(const string &);
	string getParentValue(double);
	string getHostnameWithoutPublicSuffix(const string &);
};
...
public:
	...
	vector<HmmFeature *> m_documentHmmFeaturePath;

	unsigned long m_timestamp;

	Document *m_document;
	KURL m_documentUrl;
	HmmFeature m_documentHmmFeature;

	Document *m_parentDocument;
	const void *m_parentFrameNode;

	pthread_mutex_t m_lock;
	pthread_mutexattr_t m_attr;

	bool m_isParsingFinished;

	map<const void *, const void *> m_timers;
	map<const void *, map<string, vector<pair<const void *, const void *> > > > m_eventListenerScriptMapping;

	vector<InclusionNode *> m_executingScripts;
	vector<InclusionNode *> m_preloadQueue;
	vector<InclusionNode *> m_executionQueue;

	map<const void *, InclusionNode *> m_inclusionNodes;

	int m_extensionEventId;

	Inclusion(Document *, const KURL &);
	~Inclusion();

	void preload(const string &);
	bool add(const void *, const void *, const string &, const KURL &);
	void addTimer(const void *);
	void addEvent(const void *, const string &, const void *);
	void removeEvent(const void *, const string &, const void *);
	void updateEvent(const void *);
	const void *findExtensionAncestor(const void *);
	void startExecutingScript(const void *);
	void finishExecutingScript(const void *);
	void startExecutingTimer(const void *);
	void finishExecutingTimer(const void *);
	void startExecutingEvent(const void *, const string &, const void *);
	void finishExecutingEvent(const void *, const string &, const void *);
	void getHmmFeaturePath(const void *, vector<HmmFeature *> &);
	string nodeToString(const InclusionNode *);
	void addAttribute(const void *, const string &, const string &);
	string sha256(const string &);
	void print();
	static bool sort(InclusionNode *, InclusionNode*);
};
// Excision >>>>
