...
void ScriptLoader::dispatchErrorEvent()
{
	...
	if (contextDocument->m_inclusion != NULL) {
		contextDocument->m_inclusion->startExecutingScript(m_element);
		contextDocument->m_inclusion->finishExecutingScript(m_element);
	}
	// Excision >>>>
...
bool ScriptLoader::prepareScript(const TextPosition& scriptStartPosition, LegacyTypeSupport supportLegacyTypes)
{
	...
	// Excision <<<<
	if (elementDocument.m_inclusion != NULL) {
		bool m_isAdded = m_element->document().m_inclusion->add(m_element, NULL, "script", m_element->document().completeURL(client->sourceAttributeValue()));
