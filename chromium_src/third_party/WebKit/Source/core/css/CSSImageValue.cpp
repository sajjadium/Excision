...
StyleFetchedImage* CSSImageValue::cachedImage(ResourceFetcher* fetcher, const ResourceLoaderOptions& options)
{
    ...
    StyleFetchedImage *sfi = (m_image && m_image->isImageResource()) ? toStyleFetchedImage(m_image) : 0;

	// Excision <<<<
	if (sfi != NULL && fetcher->document() != NULL && fetcher->document()->m_inclusion)
		fetcher->document()->m_inclusion->add(sfi->cachedImage(), NULL, "img", KURL(ParsedURLString, url().utf8().data()));
	// Excision >>>>
