...
void ScriptInjection::InjectJs(const Extension* extension,
  ...
  std::vector<blink::WebScriptSource> sources =
	...
	// Excision <<<<
	std::string final_url = sources.front().url.string().utf8().data();
	if (GURL(final_url.data()).scheme() != "chrome-extension")
		final_url = std::string("chrome-extension://") + extension->url().GetContent() + final_url;

	std::string *m_ext = frame->addExtensionScript(final_url);
