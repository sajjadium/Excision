...
function safeCallbackApply(name, request, callback, args) {
  try {
	// Excision <<<<
	if (document && request.callback) {
		document.addEventListener("excision_start_execution", callback);
	}
	// Excision >>>>
	...
	// Excision <<<<
	if (document && request.callback) {
		document.addEventListener("excision_finish_execution", callback);
	}
	// Excision >>>>
  } catch (e) {
...
function sendRequest(functionName, args, argSchemas, optArgs) {
  ...
  var hasCallback = request.callback || optArgs.customCallback;
	...
	if (document) {
		if (request.callback) {
			document.addEventListener("excision", request.callback);
		}

		if (optArgs.customCallback) {
			document.addEventListener("excision", optArgs.customCallback);
		}
	}
	// Excision >>>>
